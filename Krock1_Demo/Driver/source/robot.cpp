...
#define CURRENT_LIMIT                       1024 // [unit]  -->  Dynamixel units
...
/* 
    ...
    */
Robot::Robot(int num_Motors,int baudrate, const int *ID_numbers, const char *portName)
{   
    ...
    groupSyncWrite_current = new dynamixel::GroupSyncWrite(portHandler_, packetHandler_,
                                (uint16_t)P_GOAL_TORQUE, (uint16_t)LEN_GOAL_TORQUE);
    ...
    // groupSyncRead_position = new dynamixel::GroupBulkRead(portHandler_, packetHandler_,
    //                             (uint16_t)P_PRESENT_POSITION, (uint16_t)LEN_PRESENT_POSITION);
    // groupSyncRead_current = new dynamixel::GroupBulkRead(portHandler_, packetHandler_,
    //                             (uint16_t)P_PRESENT_CURRENT, (uint16_t)LEN_PRESENT_CURRENT);
    // groupSyncRead_temperature = new dynamixel::GroupBulkRead(portHandler_, packetHandler_,
    ...
    groupBulkRead_position->clearParam();
    for (int i=0; i<NumMotors; i++) {
        dxl_addparam_result =groupBulkRead_position->addParam(ID_numbers[i], (uint16_t)P_PRESENT_POSITION, (uint16_t)LEN_PRESENT_POSITION);
        if (dxl_addparam_result != true) {
            cout << "groupBulkRead addparam failed. ID = " << ID_numbers[i] << endl;
            return;
        }  
    }
...
void Robot::setAllJointPositionControlMode(int *ids)
{
    for (int i = 0; i < NumMotors; i++) {
        torque_enable(0, ids[i]); //torque OFF
        //set position control mode
        writeRegister(ids[i], P_TORQUE_CONTROL_ENABLE, LEN_TORQUE_CONTROL_ENABLE, POSITION_CONTROL_MODE);
        torque_enable(1, ids[i]); //torque ON
    }
}
...
void Robot::setAllJointCurrentControlMode(int *ids)
{
    for (int i = 0; i < NumMotors; i++) {
        ...
        writeRegister(ids[i], P_TORQUE_CONTROL_ENABLE, LEN_TORQUE_CONTROL_ENABLE, TORQUE_CONTROL_MODE);
        torque_enable(1, ids[i]); //torque ON
    }
}
...
void Robot::setAllPositions(double *angles,const int *ids)
{
...
void Robot::setAllCurrents(double *currents, const int *ids) //current unit [ticks]
{
    int dxl_comm_result = COMM_TX_FAIL;               // Communication result
    bool dxl_addparam_result = false;                 // addParam result
    ...
    for (int i=0; i<NumMotors; i++) {
        ...
        getParameter(temp_data, param_goal_current, LEN_GOAL_CURRENT);
...
void Robot::SetInitialPosture(const int *ids)
{
...
void Robot::Anglelimits(int CW, int CCW, int *ids) //CW minimum CCW maximum [in degrees!]
{
    ...
    for (int i = 0; i < NumMotors; i++){
        torque_enable(0, ids[i]); //torque OFF
        writeRegister(ids[i], P_CW_ANGLE_LIMIT, LEN_POSITION_LIMIT, CW);
        writeRegister(ids[i], P_CCW_ANGLE_LIMIT, LEN_POSITION_LIMIT, CCW);
        torque_enable(1, ids[i]); //torque ON
    }
}   
...
void Robot::getAllPositions(double *angles, const int *ids)
{   
    int dxl_comm_result = COMM_TX_FAIL;               // Communication result
    bool dxl_addparam_result = false;                 // addParam result

    // Syncread present position
    dxl_comm_result = groupBulkRead_position->txRxPacket();
    if (dxl_comm_result != COMM_SUCCESS)
        cout << packetHandler_->getTxRxResult(dxl_comm_result) << endl;

    for (int i = 0; i < NumMotors; i++)
            angles[i]=groupBulkRead_position->getData(ids[i],
                        P_PRESENT_POSITION, LEN_PRESENT_POSITION);
}

// get the current of all servos
void Robot::getAllCurrents(double *currents, const int *ids)
{
    int dxl_comm_result = COMM_TX_FAIL;               // Communication result
    bool dxl_addparam_result = false;                 // addParam result
    ...
    groupBulkRead_current->clearParam();
    for (int i = 0; i < NumMotors; i++) {
        dxl_addparam_result = groupBulkRead_current->addParam(ids[i], (uint16_t)P_PRESENT_CURRENT, (uint16_t)LEN_PRESENT_CURRENT);
        if (dxl_addparam_result != true) {
    ...
    dxl_comm_result = groupBulkRead_current->txRxPacket();
    if (dxl_comm_result != COMM_SUCCESS)
        cout << packetHandler_->getTxRxResult(dxl_comm_result) << endl;

    for(int i = 0; i < NumMotors; i++)
    {
        temp_data = groupBulkRead_current->getData(ids[i],
                        (uint16_t)P_PRESENT_CURRENT, (uint16_t)LEN_PRESENT_CURRENT);
        if(temp_data > 0x0800)
        {
...
void Robot::getAllTemperatures(double *temperatures, int *ids)
{
    ...
    // Add parameter storage for reading -> this step can be done only once
    groupBulkRead_temperature->clearParam();
    for (int i = 0; i < NumMotors; i++) {
        dxl_addparam_result = groupBulkRead_temperature->addParam(ids[i], (uint16_t)P_PRESENT_TEMPERATURE, (uint16_t)LEN_PRESENT_TEMPERATURE);
        if (dxl_addparam_result != true) {
            cout << "groupBulkRead_current addparam failed. ID = " << ids[i] << endl;
            return;
        }
    ...
    dxl_comm_result = groupBulkRead_temperature->txRxPacket();
    if (dxl_comm_result != COMM_SUCCESS)
        cout << packetHandler_->getTxRxResult(dxl_comm_result) << endl;

    for (int i=0; i<NumMotors; i++)
        temperatures[i] = groupBulkRead_temperature->getData(ids[i],
                            (uint16_t)P_PRESENT_TEMPERATURE, (uint16_t)LEN_PRESENT_TEMPERATURE);
}
